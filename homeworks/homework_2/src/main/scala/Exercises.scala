object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var temp = number
        for (i <- 2 to Math.round(Math.sqrt(temp)).asInstanceOf[Int]+1)
        {
            while(temp%i==0)
            {
                result:+=i.asInstanceOf[Int]
                temp/=i
            }
        }
        if(result.isEmpty && number!=1)
            result:+=number
        result.distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val sortedMap = ballsArray.toSeq.sortBy(x => 4.0/3 * Math.PI * Math.pow(x._2._1,3) * x._2._2 )
        for (ball <- sortedMap)
            yield ball._1
    }
